...
const SeasonProducts = () => {
  ...
  const [products,setProducts] = useState<any>([]);
    const {data : newProducts,isLoading,error : isError} = useGetTopSelledProducts(skip);
    useEffect(() => {
  ...
  return (
    <div className='flex flex-col items-center justify-center mt-4'>
        ...
        <ProductList buttonLoading={isLoading} loadMore={loadMore} products={products} isError={isError ? true : false} productLoading={products.length === 0}/>
    </div>
  )
}
