...
const Wishlist = () => {
    ...
    if(isError || products?.length === 0){
        ...
        <div className=" w-full h-[60vh] sm:h-auto sm:p-10 p-5 flex-col flex justify-center items-center">
            <img loading='lazy' src="https://img.freepik.com/free-vector/no-data-concept-illustration_114360-2506.jpg?w=740&amp;t=st=1703961108~exp=1703961708~hmac=1144a81753ebea9c937ae58ad507639a51a9bb7697a8d0956ddc0fbd54d6046c" alt="wishlist is empty" className=" w-auto h-60"/>
    ...
    if (isLoading || !isFetched) {
        return  (
            <>
                <Meta
          title="Wishlist - Sara-Ecommerce"
          description="View and manage your wishlist on Sara-Ecommerce. Keep track of your favorite products and easily access them for future purchases."
          keywords="Wishlist, Sara-Ecommerce, Favorite Products, Wishlist Management"
          />
        <WishlistSkeleton/>
            </>
        )
        ...
        <div className='flex flex-col justify-center items-center sm:gap-15 lg:mx-4 sm:w-[85%]'>
            ...
            <div className='max-w-[100%] justify-center gap-3 overflow-x-scroll grid grid-cols-2 lg:grid-cols-4'>
                   {products && products.length>0 &&  products.map((product: IWishlistProduct, index: number) => (
                    ...
                    <img loading='lazy' src={product.imageUrl} className='sm:max-w-[250px] bg-cover' alt='' />
                    ...
                    <div className='w-full flex justify-between gap-1 items-center sm:flex-row flex-col'>
                        <button aria-label='Remove Wishlist' onClick={() => removeFromWishlist({wishlistId : product._id})} className='sm:w-[48%] w-full text-xs p-1 border text-red-600 font-semibold duration-500 border-red-600 hover:text-red-600'>
                        Remove
                        </button>
                        <button aria-label='View Product Details' onClick={() => navigate(`/product/${product.productId}`)} className='sm:w-[48%] w-full text-xs p-1 border hover:bg-slate-800 font-semibold duration-500 border-slate-800 hover:text-white'>
                        View Details
                        </button>
                    </div>
                    </div>
