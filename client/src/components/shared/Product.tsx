...
const Product = ({product} : {product : IProduct}) => {
  ...
  return (
    <div key={product._id}>
                    <img src={product.imageUrl} className=' hover:scale-105 transition-all bg-cover max-h-76'  alt="" />
                    ...
                    <div className='flex justify-between items-center px-1 my-2 whitespace-nowrap'>
                        <p className='font-bold text-xs sm:text-sm xl:text-base mr-1'>₹{product?.discountedPrice}</p>
                        <p className=''>
                          <span className='text-slate-400 text-xs sm:text-sm xl:text-base line-through mr-1'>₹{product.originalPrice}</span>
                          <span className='text-green-600 text-xs sm:text-sm xl:text-base font-bold'>(33% off)</span>
                        </p>
                    </div>
